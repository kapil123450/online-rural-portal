...
class reagionwiseState extends State<reagonwise> {
  ...
  Map<String, List<DocumentSnapshot>> documents_map =
      new Map<String, List<DocumentSnapshot>>();
  List<String> postal_code = [];

  Future<String> getDocuments() async {
    Map<String, List<DocumentSnapshot>> documents_map1 =
        new Map<String, List<DocumentSnapshot>>();
    ...
    for (var i = 0; i < documents.length; i++) {
      ...
      if (postal_code1 != null) {
        for (var i1 = 0; i1 < postal_code1.length; i1++) {
          if (postal_code1[i1] == documents[i]['postalcode']) fg = false;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title:
            Text(AppLocalizations.of(context).translate("District Problems")) !=
                    null
                ? Text(
                    AppLocalizations.of(context).translate("District Problems"))
                : Text("District Problems"),
      ),
      body: postal_code.length > 0
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      child: Text(AppLocalizations.of(context).translate(
                              "view the problems of district with postal code") +
                          " : \n\n\n            " +
                          postal_code[index]),
                    ),
                  ),
          ...
          : Text(
              AppLocalizations.of(context).translate("Please refresh below")),
