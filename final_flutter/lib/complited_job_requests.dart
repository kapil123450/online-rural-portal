...
class completed_job_requests extends StatefulWidget {
  @override
  completed_job_requests_state createState() =>
      new completed_job_requests_state();
}

class completed_job_requests_state extends State<completed_job_requests> {
  ...
  Future getData() async {
    ...
    setState(() {
      for (var i = 0; i < docs.length; i++) {
        if (docs[i]['status'] == "3") documents.add(docs[i]);
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("Completed Job Requests"),
      ),
